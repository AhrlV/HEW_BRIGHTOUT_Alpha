...
=============================================================================================================*/
void Material::UpdatePixelShaderPerMaterial(PixelShader* pixelShader) const
{
    if (!pixelShader)
    {
        throw std::runtime_error("Material::UpdatePixelShaderPerMaterial - pixelShaderがnullptrです");
    }
    ...
    if (!cb)
    {
        throw std::runtime_error("Material::UpdatePixelShaderPerMaterial - PerMaterial定数バッファが見つかりません");
    }
    ...
    if (cb->HasVariable("base_color"))
    {
        ...
        memcpy(bufferData.data() + offset, &BaseColor, sizeof(DirectX::XMFLOAT4));
    }
    
    if (cb->HasVariable("diffuse_color"))
    {
        UINT offset = cb->GetVariableOffset("diffuse_color");
        memcpy(bufferData.data() + offset, &DiffuseColor, sizeof(DirectX::XMFLOAT4));
    }
