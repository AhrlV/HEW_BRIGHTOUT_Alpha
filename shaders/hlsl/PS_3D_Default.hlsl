cbuffer PerFrame : register(b0)
{
    float4 ambient_color; // 環境光の色
    float4 directional_color; // 拡散反射光の色
    float4 directional_direction; // 拡散反射光の方向（ワールド空間）
};

cbuffer PerMaterial : register(b1)
{
    float4 base_color; // 材質の拡散反射色
    float4 diffuse_color; // 拡散反射光の色
}

    
struct PS_INPUT
{
    float4 posH : SV_POSITION;
    float3 normalW : NORMAL0;
    float2 uv : TEXCOORD0;
    float4 color : COLOR0;
};


Texture2D major_texture; //　テクスチャ
SamplerState major_sampler; // テクスチャサンプラ

float4 main(PS_INPUT psin) : SV_TARGET
{    
    float4 material = major_texture.Sample(major_sampler, psin.uv) * psin.color * diffuse_color;
    float3 ambient = material.rgb * ambient_color.rgb;
        
    // -1 ~ 1 -> 0 ~ 1
    //float brightness = max(0.0f, dot(-diffuse_world_vector.xyz, normalize(psin.normalW)));
        
    // -1 ~ 1 -> 0 ~ 2 -> 0 ~ 1
    float brightness = (dot(-directional_direction.xyz, normalize(psin.normalW)) + 1) * 0.5f;
        
    float3 diffuse = material.rgb * diffuse_color.rgb * brightness;
        
    float alpha = material.a /* * ambient_color.a */;
        
    return float4(ambient + diffuse, alpha);
}